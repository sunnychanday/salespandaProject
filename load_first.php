...
echo	$csqry = "select * from sp_case_study where client_id='".$clientId."' and valid=1 and deleted=0 ORDER BY id desc  LIMIT 15";
	...
	//	$cstitleUrl = $csname_urlname."_".$caseStudyId;
		...
		if($total_tag>=1){
			...
			$resm = mysql_query($tagbyname) or die(mysql_error());
			$resData = mysql_fetch_array($resm);
		...
		if($total_liked>=1){
			...
			$like_m = mysql_query($likebyname) or die(mysql_error());
			$likeData = mysql_fetch_array($like_m);
	...
	$like_unlike_query=mysql_query($t="select * from sp_article_liked where member_id='".$userid."' and case_study_id='".$caseStudyId."' and valid=1 and deleted=0");
	$like_row=mysql_fetch_array($like_unlike_query);
	
		$article_like_id=$like_row["case_study_id"];
		$article_user_id=$like_row["member_id"];
		
	$cs_categ=mysql_query($csc="SELECT * from sp_case_study_category where user_id='".$userid."' AND case_study_id='".$caseStudyId."' AND valid=1 AND deleted=0 AND tag_retag_id='R'")or die("error in case study category table".mysql_error());
	$cs_cat_row=mysql_fetch_array($cs_categ);
	 $cs_cid=$cs_cat_row["case_study_id"];
	$cs_catUid=$cs_cat_row["user_id"];
	
	$cs_subcat=mysql_query($css="SELECT * FROM sp_case_study_subcategory WHERE user_id='".$userid."' AND case_study_id='".$caseStudyId."' AND valid=1 AND deleted=0 AND tag_retag_id='R'")or die("error in case study subcategory table".mysql_error());
	$cs_subcat_row=mysql_fetch_array($cs_subcat);
	$cs_sid=$cs_subcat_row["case_study_id"];
	$cs_subcatUid=$cs_subcat_row["user_id"];
	
	$cs_product=mysql_query($csp="SELECT * FROM sp_case_study_product WHERE user_id='".$userid."' AND case_study_id='".$caseStudyId."' AND valid=1 AND deleted=0 AND tag_retag_id='R'")or die("error in case study product table".mysql_error());
	$cs_product_row=mysql_fetch_array($cs_product);
	$cs_pid=$cs_product_row["case_study_id"];
	$cs_prdUid=$cs_product_row["user_id"];
	
	$cs_vertical=mysql_query($csv="SELECT * FROM sp_case_study_vertival WHERE user_id='".$userid."' AND case_study_id='".$caseStudyId."' AND valid=1 AND deleted=0 AND tag_retag_id='R'")or die("error in case study vertical table".mysql_error());
	$cs_vertical_row=mysql_fetch_array($cs_vertical);
...
?>
    ...
    <?php /*?><div class="tagLike">
		<?php if($userid > 0)
        	...
        	<?php if($thumbimage!=''){?>
			<?php if($castStudyUrl_link!='' && $castStudyUrl_link_url==''){ ?>
	  		<a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } else if($castStudyUrl_link_url!='') { ?>
			<a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } else {?>
            <a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } ?>
			
			<img class="thumb" src="<?php echo $sitepath; ?>upload/casestudy/thumb/<?php echo $thumbimage; ?>" id="<?php echo $caseStudyId; ?>"/><?php }  else {?>
            <input name="articleId" type="hidden" value="<?php echo $caseStudyId ; ?>" id="articleId" />
			<?php if($castStudyUrl_link!='' && $castStudyUrl_link_url==''){ ?>
			<a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } else if($castStudyUrl_link_url!='' && $castStudyUrl_link=='') { ?>
			<a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } else {?>
            <a href="showcase.php?csId=<?php echo $caseStudyId; ?>">
			<?php } ?>
...
<p class="tagCount bdrT">
            	<?php /*?><span class="floatL posRel t">
			...
			<span class="whiteTxt">Tagged by :</span> <?php if($total_tag>=1){ echo $taggedMember; } ?> <?php if($total_tag>1){ echo 'and'; ?> <?php echo $total_tag-1?> other <?php } ?></a></span></span>
            	...
            	<?php /*?><span class="floatR posRel l">
