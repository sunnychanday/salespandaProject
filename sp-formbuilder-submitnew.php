...
$message = "";
$addLeadVar = false;
$getAttempts = getAttempts('MicrositePage');
if (isset($getAttempts['attempts']) && $getAttempts['attempts'] >= 2) {
    $captcha = $_POST['g-recaptcha-response'];
    if($captcha !=''){
		$secretKey = "6LfttIwaAAAAAPFBFL_akQQBOkVsQTNTjD6md7rm";
		...
		if($responseKeys["success"] == '1'  && $responseKeys["score"] >= 0.5) {
			enableAttempts('MicrositePage');
			$addLeadVar = true;
		}else{
			$addLeadVar = false;
			$message = "Google captcha seding error".$responseKeys['error'];
		}
    } else {
        $addLeadVar = false;
        $message = "Google captcha token is not getting passed";
    }
} else {

	$addLeadVar = false;
    $message = "Attempted more than 2 times";
}
...
if ($addLeadVar === true) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $chkm = "select * from sp_contact where client_id='" . $client_id . "' and (email_id='" . $el . "' OR mobile='" . $mle . "') and valid=1 and deleted=0";
        ...
        if ($countcont > 0) {
            ...
            $fn = (!empty($fn)) ? $fn: $contactData['first_name'];
            $stateId = (!empty($stateId)) ? $stateId: $contactData['state'];
            $address = (!empty($address)) ? $address: $contactData['address'];
            $cityId = (!empty($cityId)) ? $cityId: $contactData['city'];
            $pincode = (!empty($pincode)) ? $pincode: $contactData['pincode'];
            $compId = (!empty($compId)) ? $compId: $contactData['comp_id'];
            $plan_segment = (!empty($plan_segment)) ? $plan_segment: $contactData['custom_field2'];
            $your_message = (!empty($your_message)) ? $your_message: $contactData['custom_field3'];
            $plan_subarea = (!empty($plan_subarea)) ? $plan_subarea: $contactData['custom_field4'];
            $plan_name = (!empty($plan_name)) ? $plan_name: $contactData['custom_field5'];
            $building_name = (!empty($building_name)) ? $building_name: $contactData['custom_field6'];
            $building_no = (!empty($building_no)) ? $building_no: $contactData['custom_field7'];
            $adr_latitude = (!empty($adr_latitude)) ? $adr_latitude: $contactData['lat_custom_field'];
            $adr_longitude = (!empty($adr_longitude)) ? $adr_longitude: $contactData['lng_custom_field'];
